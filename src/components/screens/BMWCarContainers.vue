    ...
    <div v-else-if="step === 'connected'">
      ...
      <div v-if="containersList.length" class="card table-card">
        ...
        <table class="data-table">
          ...
          <tbody>
            <tr v-for="c in containersList" :key="c.containerId">
              <td>
                <code>{{ c.containerId }}</code>
              </td>
              <td>{{ c.name || "N/A" }}</td>
              <td>
              ...
              <td class="actions-cell">
                <button
                  ...
                  @click="getContainerDetail(c.containerId)"
                >
                  Details
                </button>
                <button
                  class="btn small danger"
                  @click="deleteContainer(c.containerId)"
                >
                  Delete
                </button>
                <button
                  class="btn small danger"
                  @click="telematic(c.containerId)"
                >
                  Telematics
                </button>
              </td>
...
export default {
  ...
  methods: {
    ...
    async telematic(containerId) {
      this.isLoading = true;

      const response = await window.ipc.receive("api", {
        method: "get",
        endpoint: `bmw/cardata/telematics`,
        options: {
          useAPIKey: true,
        },
        body: { containerId, vin: "WBA6N320X0FK24381" },
      });
